...
def send_verification_email(request, user_email):
    ...
    send_email(
        ...
        message=f"Hi {user.get_full_name()}.\n\tThank you for registering at 'Django-Auth-APP'. Use the follow code to activate your account.\n\t{
            code}. Or follow next link:\n\t{base_url}verify?code={code} .",
        to_email=user.email,
    )
    return
...
def send_password_reset_email(context, user):
    ...
    base_url = request.build_absolute_uri('password/')
    send_email(
        subject="Commerce: Reset your password.",
        message=f"Hi {user.get_full_name()}.\n\tThank you for registering at 'Django-Auth-APP'. Use the follow code to reset your password.\n\tBase: {
            codes[0]}\n\tCode: {codes[1]}\n. Or follow next link:\n\t{base_url}verify/{codes[0]}/{codes[1]} .",
        to_email=user.email,
    )
    return
